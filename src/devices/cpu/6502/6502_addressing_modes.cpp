...
uint8_t cpu::ABS ()
{
    ...
    this -> destination_address = 0x0000;
    aux                         = this -> read (this -> program_counter);
    (this -> program_counter)++;
    this -> destination_address += aux;
    this -> destination_address <<= 8;
    aux                         = this -> read (this -> program_counter);
    (this -> program_counter)++;
    this -> destination_address += aux;
...
uint8_t cpu::ABX ()
{
    ...
    (this -> program_counter)++;
    this -> destination_address += aux_1;
    this -> destination_address <<= 8;
    aux_2                       = this -> read (this -> program_counter);
    (this -> program_counter)++;
    this -> destination_address += aux_2;

    this -> destination_address += this -> x_register;

    if ((this -> destination_address >> 8) != aux_1)
        return (1);
...
uint8_t cpu::IDX ()
{
    ...
    aux_1 = this -> read ((aux_adr + (this -> x_register)) & 0x00FF);
    aux_2 = this -> read ((aux_adr + (this -> x_register) + 1) & 0x00FF);
...
uint8_t cpu::IDY ()
{
    ...
    if (((this -> destination_address) & 0xFF00) != aux_2)
        return (1);
